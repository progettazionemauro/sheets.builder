...
def builder_state_to_runtime_data(builder_state: Dict[str, Any]) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    required_on_insert = [f["name"] for f in fields if bool(f.get("required")) and not bool(f.get("computed"))]
    optional_on_insert = [f["name"] for f in fields if not bool(f.get("required")) and not bool(f.get("computed"))]
    computed = [f["name"] for f in fields if bool(f.get("computed"))]
    visible_in_form = [f["name"] for f in fields if bool(f.get("visibleInForm"))]
    visible_in_viewer = [f["name"] for f in fields if bool(f.get("visibleInViewer"))]
...
@app.get("/health")
def health():
    return jsonify({"ok": True, "service": "sheet-builder-flask"})
...
def api_parse():
    try:
        ...
        if not builder_state["project"].get("projectSlug"):
            builder_state["project"]["projectSlug"] = sheet_name.strip().lower().replace(" ", "-")
...
def api_generate():
    try:
        ...
        if not isinstance(builder_state, dict):
            return jsonify({"ok": False, "error": "Missing or invalid builder_state"}), 400
        ...
        project_config, fields_schema = builder_state_to_runtime_data(builder_state)
