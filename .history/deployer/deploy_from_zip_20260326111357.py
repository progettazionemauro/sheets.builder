...
def enforce_target_mode(repo_dir: Path, target_mode: str) -> None:
    """
    new:
      - la cartella target non deve esistere
    overwrite:
      - la cartella target può esistere
    """
    if target_mode == "new":
        if repo_dir.exists():
            raise RuntimeError(
        ...
        # nessun blocco: se esiste la usiamo, se non esiste la creiamo
        return
...
def main() -> int:
    ...
    try:
        ...
        if args.prepare_frontend_repo:
            ...
            print("4. Il flusso GAS/clasp sarà il passo successivo.")
