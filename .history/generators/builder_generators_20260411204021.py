...
def _normalize_formula_for_gs_template(formula: str, target_row_var: str = "rowNumber") -> str:
    """
    Trasforma riferimenti tipo C2, D2, $A$1, B$3, $C4
    mantenendo le colonne e rimpiazzando la parte relativa di riga
    con ${rowNumber} o ${row}.
    Questa è una base utile per formule row-based.
    """
    if not formula:
        return ""
...
def _generate_backend_computed_formula_write(fields: List[FieldDef], mode: str) -> str:
    ...
    for idx, f in enumerate(fields, start=1):
        ...
        if f.formula_source and f.formula_mode == "incremental_copy":
            normalized = _normalize_formula_for_gs_template(f.formula_source, target_var=target_var)
            js_formula = normalized.replace("\\", "\\\\").replace("`", "\\`")
            lines.append(
...
{insert_validations}
  ...
  {insert_formula_write if insert_formula_write else ""}
...
{update_formula_write if update_formula_write else ""}
  const out = {{ ok: true, id, updatedRow: row }};  if (debug) out.debug = {{ keys: Object.keys(p).sort() }};
  return out;
}}
