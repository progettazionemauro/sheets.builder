...
def validate_schema_for_product(fields_schema: Dict[str, Any]) -> List[str]:
    ...
    for name, f in field_by_name.items():
        ...
        # type=computed must match computed=true
        ...
        if field_type != "computed" and is_computed and name != "id":
            # Questa regola la tengo severa: se computed=true il type dovrebbe essere computed,
            # tranne per id che vogliamo int + computed=true.
            errors.append(
                f"Field '{name}' has computed=true but type is '{field_type}'. "
                "Use type='computed' or make it non-computed."
            )

        # locked normale: ammesso, ma se non computed e non id non può sparire dal viewer in un builder severo?
        # No: non imponiamo questa regola, altrimenti diventa troppo restrittivo.
        _ = is_locked  # keep explicit for readability
    ...
    # =========================================================
    # AGGREGATED LISTS MUST MATCH FIELD DEFINITIONS
    # =========================================================
