...
def _generate_backend_extract(fields: List[FieldDef]) -> str:
    lines: List[str] = []
...
def _generate_backend_missing_checks(fields: List[FieldDef]) -> str:
    lines = ['  const missing = [];']
    for f in non_computed_fields(fields):
        var_name = slugify_for_dom(f.name)
        if f.required:
...
def _generate_backend_validations(fields: List[FieldDef], constraints: Dict[str, Any]) -> str:
    lines: List[str] = []
    for f in non_computed_fields(fields):
        var_name = slugify_for_dom(f.name)
        ...
        if f.type == "string":
            ...
            if isinstance(max_len, int):
                lines.append(
                    f'  if ({var_name} && {var_name}.length > {max_len}) return {{ ok: false, error: {js_string(f"{f.name} too long (max {max_len})")} }};'
                )

        if f.type == "enum" and f.enum_values:
            enum_const = f"ENUM_{slugify_for_dom(f.name).upper()}"
            lines.append(
                f'  if ({var_name} && {enum_const}.indexOf({var_name}) === -1) return {{ ok: false, error: {js_string(f"{f.name} not allowed")}, allowed: {enum_const} }};'
            )
...
def generate_gas_backend(project_config: Dict[str, Any], fields_schema: Dict[str, Any]) -> str:
    ...
    extract_block = _generate_backend_extract(fields)
    missing_block = _generate_backend_missing_checks(fields)
    validation_block = _generate_backend_validations(fields, constraints)
...
def _html_input_for_field(field: FieldDef, constraints: Dict[str, Any]) -> str:
    ...
    if field.type == "enum":
        ...
        options_html = "\n      ".join(options)
        return f'''    <label for="{dom_id}">{label} <span class="muted">({required_label})</span></label>
    <select id="{dom_id}">
      {options_html}
    </select>'''
    ...
    if field.type == "int":
        ...
        return f'''    <label for="{dom_id}">{label} <span class="muted">({required_label})</span></label>
    <input id="{dom_id}" type="number"{min_v}{max_v} placeholder="{label}" />'''
    ...
    return f'''    <label for="{dom_id}">{label} <span class="muted">({required_label})</span></label>
    <input id="{dom_id}" placeholder="{escape_html(placeholder)}" />'''


def _generate_form_html(fields: List[FieldDef], constraints: Dict[str, Any]) -> str:
    parts = [_html_input_for_field(f, constraints) for f in form_fields(fields)]
    return "\n\n".join(parts)
...
def _generate_payload_js(fields: List[FieldDef]) -> str:
    ...
    for f in form_fields(fields):
        ...
        if f.type == "enum":
            lines.append(
                f'        {js_string(f.name)}: document.getElementById({js_string(dom_id)}).value'
            )
        else:
            lines.append(
                f'        {js_string(f.name)}: document.getElementById({js_string(dom_id)}).value.trim()'
            )
    return "{\n" + ",\n".join(lines) + "\n      }"
...
def generate_viewer_html(project_config: Dict[str, Any], fields_schema: Dict[str, Any]) -> str:
    fields = field_map_from_schema(fields_schema)
    visible_headers = [f.name for f in viewer_fields(fields)]
...
<script>
  ...
  const VISIBLE_HEADERS = {js_pretty(visible_headers)};
