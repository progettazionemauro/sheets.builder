...
def extract_bundle(zip_path: str | Path) -> Dict[str, Optional[Path]]:
    ...
    xlsx_files = list(temp_dir.rglob("*.xlsx"))
    html_files = list(temp_dir.rglob("*.html"))
    css_files = list(temp_dir.rglob("*.css"))

    if not xlsx_files:
        raise FileNotFoundError("No .xlsx file found inside bundle")
    if not html_files:
        raise FileNotFoundError("No .html file found inside bundle")
