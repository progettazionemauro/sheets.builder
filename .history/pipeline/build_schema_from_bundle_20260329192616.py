...
def find_input_files(input_dir: str | Path) -> Dict[str, Optional[Path]]:
    """
    Cerca automaticamente nella cartella input:
    - almeno un .xlsx (obbligatorio)
    - uno .html (opzionale)
    - uno .css (opzionale)

    Ricerca ricorsiva, ma semplice.
    """
    ...
    if not xlsx_files:
        raise FileNotFoundError(
            f"No .xlsx file found inside input directory: {input_dir}"
        )
...
def main() -> None:
    """
    Uso:
      python pipeline/build_schema_from_bundle.py
      python pipeline/build_schema_from_bundle.py /path/to/input_dir
      python pipeline/build_schema_from_bundle.py /path/to/input_dir rating
    """

    if len(sys.argv) == 1:
        # default: usa examples/
