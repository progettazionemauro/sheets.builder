...
// ===== CONFIG =====
const BACKEND_NAME = "volcano-db-backend";
const SHEET_NAME = "volcano_db";
...
function doGet(e) {
  ...
  if (!cb) return ContentService.createTextOutput("Missing cb (JSONP)");
    ...
    /*
   * LEVEL B2 — persistent backend routing.
   *
   * La presenza simultanea dei tre parametri tecnici identifica
   * una richiesta instradata dal proxy persistente Djungo.
   */
  if (p.appSlug && p.spreadsheetId && p.sheetId) {
    try {
      return jsonp_(cb, djungoPersistentRequest_(p));
    } catch (err) {
      return jsonp_(cb, {
        ok: false,
...
function insert_(p, debug) {
  ...
  const volcano_name = norm_(p["volcano name"]);
  const country = norm_(p["country"]);
  const region = norm_(p["region"]);
  const latitude = norm_(p["latitude"]);
  const longitude = norm_(p["longitude"]);
  const elev = clampInt_(p["elev"], -999999999, 999999999, 0);
  const type = norm_(p["type"]);
  const status = norm_(p["status"]);
  const last_known = norm_(p["last known"]);

  const missing = [];
  if (missing.length) return { ok: false, error: "Missing required fields", missing };

  if (number && number.length > 255) return { ok: false, error: "number too long (max 255)" };
  if (volcano_name && volcano_name.length > 255) return { ok: false, error: "volcano name too long (max 255)" };
  if (country && country.length > 255) return { ok: false, error: "country too long (max 255)" };
  if (region && region.length > 255) return { ok: false, error: "region too long (max 255)" };
  if (latitude && latitude.length > 255) return { ok: false, error: "latitude too long (max 255)" };
  if (longitude && longitude.length > 255) return { ok: false, error: "longitude too long (max 255)" };
  if (elev && elev < 0) return { ok: false, error: "elev below min 0" };
  if (type && type.length > 255) return { ok: false, error: "type too long (max 255)" };
  if (status && status.length > 255) return { ok: false, error: "status too long (max 255)" };
  if (last_known && last_known.length > 255) return { ok: false, error: "last known too long (max 255)" };

  const id = computeNextId_(sh);
  const rowNumber = sh.getLastRow() + 1;

  sh.getRange(rowNumber, 1, 1, HEADERS.length).setValues([[number,
    volcano_name,
    country,
    region,
    latitude,
...
function update_(id, p, debug) {
  ...
  sh.getRange(row, 1, 1, HEADERS.length).setValues([[number,
    volcano_name,
    country,
    region,
    latitude,
